...
static uint32_t getSasTokenExpiration(const char* sasToken)
{
  ...
  if (j != sizeof(SE))
  {
    ESP_LOGE("AzureSasToken", "Failed finding `se` field in SAS token");
  }
  else
  {
    int k = i;
    ...
    {
      ESP_LOGE("AzureSasToken", "Failed parsing SAS token expiration time stamp");
    }
  }
    ...
    az_span* out_base64_encoded_bytes)
{
  ...
  if (mbedtls_base64_encode(
      ...
      != 0)
  {
    ESP_LOGE("AzureSasToken", "mbedtls_base64_encode fail");
  }
    ...
    az_span sas_token)
{
  ...
  {
    ESP_LOGE("AzureSasToken", "Could not get the signature for SAS key");
  ...
  if (iot_sample_generate_sas_base64_encoded_signed_signature(
      ...
      != 0)
  {
    ESP_LOGE("AzureSasToken", "Failed generating SAS token signed signature");
    return AZ_SPAN_EMPTY;
  }
  ...
  if (az_result_failed(rc))
  {
    ESP_LOGE("AzureSasToken", "Could not get the password");
    return AZ_SPAN_EMPTY;
  }
  else
  {
...
int AzureSasToken::Generate(unsigned int expiryTimeInMinutes)
{
  ...
  {
    ESP_LOGE("AzureSasToken", "Failed generating SAS token");
    return 1;
  }
  else
  {
    ...
    {
      ESP_LOGE("AzureSasToken", "Failed getting the SAS token expiration time");
      this->sasToken = AZ_SPAN_EMPTY;
      return 1;
    }
    else
...
bool AzureSasToken::IsExpired()
{
  time_t now = time(NULL);

  if (now == INDEFINITE_TIME)
  {
    ESP_LOGE("AzureSasToken", "Failed getting current time");
    return true;
  }
  else
  {
...
az_span AzureSasToken::Get() { return this->sasToken; }
